package com.zettamine.mpa.scm.aspects;
...
public class LoggingAspect {
     ...
     */
    @Before("execution(* com.zettamine.mpa.scm.*.*.*(..))")
     ...
     * @param joinPoint The JoinPoint representing the method being intercepted.
     */
    @After("execution(* com.zettamine.mpa.scm.*.*.*(..))")
    public void logAfterMethodCall(JoinPoint joinPoint) {
        log.info(ANSI_GREEN + "Method - \"" + joinPoint.getSignature().getName() + "()\" has executed" + ANSI_RESET);
    }
     ...
     * @param joinPoint The JoinPoint representing the method being intercepted.
     */
    @AfterReturning(pointcut = "execution(* com.zettamine.mpa.scm.*.*.*(..))", returning = "returnValue")
    public void logAfterSuccessfulMethodCall(JoinPoint joinPoint) {
        log.info(ANSI_GREEN + "Execution Completed for \"" + joinPoint.getSignature().getName() + "()\" with " + joinPoint.getArgs().length + " arguments and returns - " + ANSI_RESET);
    }
     ...
     */
    @AfterThrowing(pointcut = "execution(* com.zettamine.mpa.scm.*.*.*(..))", throwing = "ex")
    public void logMethodCallAfterException(JoinPoint joinPoint, Exception ex) {
        log.info(ANSI_RED + joinPoint.getSignature().getName() + "() fired an exception -> " + ex.toString() + ANSI_RESET);
    }
}
